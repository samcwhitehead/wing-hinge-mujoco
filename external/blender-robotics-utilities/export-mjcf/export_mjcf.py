...
### SCW edits
# commented out line that asserts joints per child <= 1
# trying to switch from mass to density
...
def get_joints(obj):
    ...
    # check DOF rather than joint count (SCW)
    rot_dof = sum(
        [(j.enum_joint_type == 'hinge') + 3 * (j.enum_joint_type == 'ball') + 3 * (j.enum_joint_type == 'free') for j in
         joints])
    trans_dof = sum([(j.enum_joint_type == 'slide') + 3 * (j.enum_joint_type == 'free') for j in joints])
    assert rot_dof <= 3, "too many rotational degrees of freedom in TF tree build"
    assert trans_dof <= 3, "too many translational degrees of freedom in TF tree build"
    # assert len(joints) < 2, "too many child joints detected in TF tree build" # TODO check num DOF rather than joint
    return joints
...
def export_mesh_geom(context, obj, xml, xml_asset, visualization_only=False):
    ...
    if visualization_only:
        # xml_geom.set("mass", "0")
        xml_geom.set("mass", "0")  # density=0 causes errors, I think because of moment of inertia issues?
        xml_geom.set("contype", "0")
        xml_geom.set("conaffinity", "0")
    else:
        # xml_geom.set("mass", repr(obj.sk_mass))
        xml_geom.set("density", repr(obj.sk_density))
...
def export_entity(context, obj, xml_model, body_is_root, xml_asset):
    ...
    if obj.enum_sk_type == "geom":
        ...
        xml_entity.set("density", repr(obj.sk_density))
        ...
        if body_is_root:
            ...
            # always export an STL mesh, but only let it be the physics geom
    ...
    def draw(self, context):
        ...
        if obj.enum_sk_type == 'body':
            ...
            if obj.sk_use_primitive_geom:
                ...
                row.prop(obj, 'sk_density', text='Density')
...
def register():
    ...
    bpy.types.Object.sk_density = bpy.props.FloatProperty(name='sk_density', default=1000)
    bpy.types.Object.sk_use_primitive_geom = bpy.props.BoolProperty(name="use_primitive_geom", default=False)
...
def unregister():
    ...
    del bpy.types.Object.sk_density
