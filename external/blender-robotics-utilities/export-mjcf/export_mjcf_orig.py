...
class TFDisplay():
    ...
    def tf_display_update(self, scene, unknown):
        ...
        if scene.kinematic_tree_display:
            self.draw_handler = bpy.types.SpaceView3D.draw_handler_add(self.tf_display_draw, (scene,), 'WINDOW',
                                                                       'POST_VIEW')
...
def build_kinematic_tree(self, context):
    ...
    for obj in context.scene.objects:
        if (["body", "joint", "geom", "site", "camera", "sensor"].count(obj.enum_sk_type) > 0) and not (
                obj.sk_parent_entity == None):
...
def export_stl(context, obj, xml_geom, xml_asset):
    ...
    if obj.data not in meshes_exported.keys():
        ...
        if mesh_export_name in meshes_exported.values():
            print(
                "[WARNING] Detected mesh name collision! Will auto-resolve, but there should be no expectation of stability in the resulting names.")
        ...
        bpy.ops.export_mesh.stl(filepath=bpy.path.abspath('//mesh_stl/' + mesh_export_name + '.stl'),
                                use_selection=True, global_space=obj.matrix_world, ascii=False)
...
def export_options(context, xml_root):  # TODO add lower priority options here and to the UI
    ...
    if context.scene.mjcf_flag_contactparam_override:
        xml_options.set("o_solref",
                        f"{context.scene.mjcf_option_osolref[0]:.4} {context.scene.mjcf_option_osolref[1]:.4}")
        xml_options.set("o_solimp",
                        f"{context.scene.mjcf_option_osolimp[0]:.4} {context.scene.mjcf_option_osolimp[1]:.4} {context.scene.mjcf_option_osolimp[2]:.4}")
...
def export_actuators(context, xml):
    ...
    for act in actuator_list:
        ...
        if act.sk_actuator_ctrllimited:
            xml_act.set("ctrlrange",
                        repr(act.sk_actuator_ctrllimit_lower) + " " + repr(act.sk_actuator_ctrllimit_upper))

        xml_act.set("forcelimited", repr(act.sk_actuator_forcelimited).lower())
        if act.sk_actuator_forcelimited:
            xml_act.set("forcerange",
                        repr(act.sk_actuator_forcelimit_lower) + " " + repr(act.sk_actuator_forcelimit_upper))
...
def export_equalities(context, xml):
    ...
    for eq in equality_list:
        ...
        if eq.sk_equality_type == "tendon" or eq.sk_equality_type == "joint":
            ...
            xml_eq.set("polycoef",
                       f"{eq.sk_equality_polycoef[0]} {eq.sk_equality_polycoef[1]} {eq.sk_equality_polycoef[2]} {eq.sk_equality_polycoef[3]} {eq.sk_equality_polycoef[4]}")
...
def export_entity(context, obj, xml_model, body_is_root, xml_asset):
    ...
    if obj.enum_sk_type == "geom":
        ...
        print(
            f"   Going to use the parent object: {parent_tmp.name}")  # this gets the correct answer - the intended body
        ...
        if obj.sk_joint_friction:
            ...
            if obj.sk_joint_solimpfriction_custom:
                xml_entity.set("solimpfriction",
                               f"{obj.sk_joint_solimpfriction[0]} {obj.sk_joint_solimpfriction[1]} {obj.sk_joint_solimpfriction[2]}")
...
class MJCFExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not (root.location[0] == 0 and root.location[1] == 0 and root.location[2] == 0):
            ...
            self.report({'WARNING'},
                        'Root object origin is not located at world origin. Transformations of objects attached to the world will be incorrect.')
        ...
        if (nuserdata_estimate > context.scene.mjcf_option_nuserdata):
            self.report({'WARNING'},
                        f"It is likely insufficient userdata is allocated. Recommend nuserdata={nuserdata_estimate}")
    ...
    def draw(self, context):
        ...
        if obj.enum_sk_type == 'body':
            ...
            if obj.sk_use_primitive_geom:
                row = layout.row()
                row.label(
                    text="Derived Mass (kg): TODO")  # if body is to built of geoms, then those should drive the mass
...
enum_equality_type = [
    ...
    3 * ("weld",),
    ("joint", "joint y=f(x)", "Constrains joint position y to be some polynomial function of joint position x."),
    # gears, differentials, etc.polynomial function
    3 * ("tendon",),
...
enum_camera_mode = [
    ...
    ("track", "Track",
     "Position is at a constant offset from the parent in world coordinates, while the camera orientation is constant in world coordinates."),
    ("trackcom", "Track CM",
     "Tracks center of mass of kinematic subtree. Position is at a constant offset from the parent in world coordinates, while the camera orientation is constant in world coordinates."),
    ("targetbody", "Target Body",
     "Camera position is fixed in the parent body, while the camera orientation is adjusted so that it always points towards the targeted body."),
    ("targetbodycom", "Target Body CM",
     "Tracks center of masss of kinematic subtree.Camera position is fixed in the parent body, while the camera orientation is adjusted so that it always points towards the targeted body. ")
]
...
mjcf_options_jacobian = [
    ("dense", "dense",
     "Normal. Selected by auto if system DOF<60. Required for manual manipulation of jacobian matrices, as done in parallel mechanisms"),
...
def register():
    ...
    bpy.types.Scene.root_previous = bpy.props.PointerProperty(type=bpy.types.Object, name="root_previous",
                                                              description="Last-used root export object", update=None)
    ...
    bpy.types.Object.sk_parent_entity = bpy.props.PointerProperty(type=bpy.types.Object, name="sk_parent_entity",
                                                                  description="Simple Kinematics Parent Entity",
                                                                  update=build_kinematic_tree_auto)
    bpy.types.Object.sk_solref = bpy.props.FloatVectorProperty(name="sk_solref", size=2, default=[0.0005, 1],
                                                               precision=4,
                                                               description="(timeconst, dampratio). Should keep [time constant] > 2*[simulation stpe time]")
    bpy.types.Object.sk_solref_custom = bpy.props.BoolProperty(name="sk_solref_custom", default=False)
    bpy.types.Object.sk_solimp = bpy.props.FloatVectorProperty(name="sk_solimp", size=3, default=[2, 1.2, 0.001],
                                                               precision=4)
    ...
    bpy.types.Object.sk_axis_lower_rot = bpy.props.FloatProperty(name="lower_rot", default=0, soft_min=-2 * pi,
                                                                 soft_max=2 * pi, unit="ROTATION", step=step_angle_ui)
    bpy.types.Object.sk_axis_upper_rot = bpy.props.FloatProperty(name="upper_rot", default=0, soft_min=-2 * pi,
                                                                 soft_max=2 * pi, unit="ROTATION", step=step_angle_ui)
    bpy.types.Object.sk_axis_lower_lin = bpy.props.FloatProperty(name="lower_lin", default=0, soft_min=-1, soft_max=1,
                                                                 unit="LENGTH", step=step_lin_ui)
    bpy.types.Object.sk_axis_upper_lin = bpy.props.FloatProperty(name="upper_lin", default=0, soft_min=-1, soft_max=1,
                                                                 unit="LENGTH", step=step_lin_ui)
    bpy.types.Object.sk_joint_stiffness = bpy.props.FloatProperty(name="stiffness", default=0, soft_min=-1000,
                                                                  soft_max=1000, unit="NONE", step=step_lin_ui)
    bpy.types.Object.sk_joint_damping = bpy.props.FloatProperty(name="damping", default=0, soft_min=-1000,
                                                                soft_max=1000, unit="NONE", step=step_lin_ui)
    bpy.types.Object.sk_joint_springref_lin = bpy.props.FloatProperty(name="springref_lin", default=0, soft_min=-1,
                                                                      soft_max=1, unit="LENGTH", step=step_lin_ui)
    bpy.types.Object.sk_joint_springref_rot = bpy.props.FloatProperty(name="springref_rot", default=0, soft_min=-1,
                                                                      soft_max=1, unit="ROTATION", step=step_angle_ui)
    ...
    bpy.types.Object.sk_joint_solreffriction = bpy.props.FloatVectorProperty(name="sk_solreffriction", size=2,
                                                                             default=[0.0005, 1], precision=4,
                                                                             description="(timeconst, dampratio). Should keep [time constant] > 2*[simulation stpe time]")
    bpy.types.Object.sk_joint_solreffriction_custom = bpy.props.BoolProperty(name="sk_solreffriction_custom",
                                                                             default=False)
    bpy.types.Object.sk_joint_solimpfriction = bpy.props.FloatVectorProperty(name="sk_solimpfriction", size=3,
                                                                             default=[2, 1.2, 0.001], precision=4)
    bpy.types.Object.sk_joint_solimpfriction_custom = bpy.props.BoolProperty(name="sk_solimpfriction_custom",
                                                                             default=False)
    bpy.types.Object.sk_joint_armature = bpy.props.FloatProperty(name="sk_joint_armature", default=0, precision=6,
                                                                 description="Armature inertia (or rotor inertia, or reflected inertia) of all degrees of freedom created by this joint.")
    ...
    bpy.types.Object.sk_actuator_ctrllimit_upper = bpy.props.FloatProperty(name="ctrllimit_upper", default=0,
                                                                           soft_min=-50, soft_max=50, unit="NONE",
                                                                           step=step_angle_ui)
    bpy.types.Object.sk_actuator_ctrllimit_lower = bpy.props.FloatProperty(name="ctrllimit_lower", default=0,
                                                                           soft_min=-50, soft_max=50, unit="NONE",
                                                                           step=step_angle_ui)
    bpy.types.Object.sk_actuator_forcelimited = bpy.props.BoolProperty(name="forcelimited", default=False)
    bpy.types.Object.sk_actuator_forcelimit_upper = bpy.props.FloatProperty(name="forcelimit_upper", default=0,
                                                                            soft_min=-50, soft_max=50, unit="NONE",
                                                                            step=step_angle_ui)
    bpy.types.Object.sk_actuator_forcelimit_lower = bpy.props.FloatProperty(name="forcelimit_lower", default=0,
                                                                            soft_min=-50, soft_max=50, unit="NONE",
                                                                            step=step_angle_ui)
    bpy.types.Object.sk_actuator_kv = bpy.props.FloatProperty(name="actuator_kv", default=1, soft_min=0, soft_max=10,
                                                              unit="NONE")
    bpy.types.Object.sk_actuator_kp = bpy.props.FloatProperty(name="actuator_kp", default=1, soft_min=0, soft_max=100,
                                                              unit="NONE")
    bpy.types.Object.sk_actuator_pid = bpy.props.FloatVectorProperty(name="actuator_pid", size=10,
                                                                     default=[0.0, 0.0, 1.0, 0.0, 0.5, 0.0, 0.0, 1.0,
                                                                              0.5, 1.0])

    # Equality Constraint Properties
    bpy.types.Object.sk_equality_type = bpy.props.EnumProperty(items=enum_equality_type)
    bpy.types.Object.sk_equality_entity1 = bpy.props.PointerProperty(type=bpy.types.Object, name="sk_equality_entity1",
                                                                     description="equality entity 1", update=None)
    bpy.types.Object.sk_equality_entity2 = bpy.props.PointerProperty(type=bpy.types.Object, name="sk_equality_entity2",
                                                                     description="equality entity 2", update=None)
    bpy.types.Object.sk_equality_polycoef = bpy.props.FloatVectorProperty(name="sk_equality_polycoef", size=5,
                                                                          description="a[i] in y-y0 = a0 + a1*(x-x0) + a2*(x-x0)^2 + a3*(x-x0)^3 + a4*(x-x0)^4")
    bpy.types.Object.sk_equality_distance = bpy.props.FloatProperty(name="sk_equality_distance", description="distance",
                                                                    unit="LENGTH")
    ...
    bpy.types.Object.sk_camera_target = bpy.props.PointerProperty(type=bpy.types.Object, name="sk_camera_target",
                                                                  description="camera target", update=None)

    # Options
    bpy.types.Scene.mjcf_option_timestep = bpy.props.FloatProperty(name="mjcf_option_timestep", description="timestep",
                                                                   unit="NONE", default=0.002, precision=3)
    bpy.types.Scene.mjcf_option_iterations = bpy.props.IntProperty(name="mjcf_option_iterations",
                                                                   description="solver iterations", default=100)
    bpy.types.Scene.mjcf_flag_contactparam_override = bpy.props.BoolProperty(name="mjcf_flag_contactparam_override",
                                                                             default=False)
    bpy.types.Scene.mjcf_option_osolref = bpy.props.FloatVectorProperty(name="mjcf_option_osolref",
                                                                        description="override solref of contacts",
                                                                        size=2, precision=3, default=[0.002, 1.0])
    bpy.types.Scene.mjcf_option_osolimp = bpy.props.FloatVectorProperty(name="mjcf_option_osolimp",
                                                                        description="override solimp of contacts",
                                                                        size=3, precision=3, default=[0.9, 0.8, 0.001])
    ...
    bpy.types.Scene.mjcf_option_nconmax = bpy.props.IntProperty(name="mjcf_option_nconmax",
                                                                description="Maximum number of contacts. -1 to automatically guess",
                                                                default=-1)
    bpy.types.Scene.mjcf_option_njmax = bpy.props.IntProperty(name="mjcf_option_njmax",
                                                              description="Maximum number of scalar constraints. -1 to automatically guess",
                                                              default=-1)
    bpy.types.Scene.mjcf_option_nuserdata = bpy.props.IntProperty(name="mjcf_option_nuserdata",
                                                                  description="Additional custom user data items",
                                                                  default=100)
    bpy.types.Scene.mjcf_option_energy = bpy.props.BoolProperty(name="mjcf_option_energy",
                                                                description="When enabled, Mujoco will compute system potential and kinetic energy",
                                                                default=False)
